...
void AggresiveAgent::init(
   GameState initialState, const ForwardModel& /*forwardModel*/, Timer /*timer*/)
{
   parameters_.PLAYER_ID = getPlayerID();
   // The heuristic is set here because some heuristics have parameters depending on the game state.
   // e.g. num_units
   /*
       if (parameters_.heuristic == nullptr)
           parameters_.heuristic = std::make_unique<AbstractHeuristic>(initialState);
   */
   parameters_.heuristic = std::make_unique< AggresiveHeuristic >(initialState, getPlayerID());
...
ActionAssignment AggresiveAgent::computeAction(
   GameState state, const ForwardModel& forwardModel, Timer timer)
{
   ...
   if(actionSpace.size() == 1 || ! parameters_.continuePreviousSearch) {
      ...
      auto bestAction = rootNode->getActionSpace(forwardModel, getPlayerID())
                           .at(static_cast< size_t >(bestActionIndex));

...
namespace SGA {
   ...
   double WizardWarsHeuristic::evaluateGameState(const ForwardModel& /*forwardModel*/, GameState& gameState, int playerID) {
      double score = 0.0;
      ...
      //Army Info
      int allyArmySize = 0; int enemyArmySize = 0;
      double allyArmyHealth = 0; double enemyArmyHealth = 0;
      ...
      for (const auto& entity : gameState.getEntities())
      {
         ...
         if (entity.getOwnerID() != gameState.getCurrentTBSPlayer())
         {
            ...
            if (entityType.getName() == "King") {
               kingX = static_cast<int>(entity.x()); kingY = static_cast<int>(entity.y());
      ...
      {
         if ((gameState.getGameInfo()->getEntityType(gameState.getEntity(entity)->getEntityTypeID()).getName() != "King")) {
      ...
      if (gameState.isGameOver())
      {
         if (gameState.getWinnerID() == playerID)
            return 10000000000000;	// since this score should be minimized we need to deduct points for winning
         else
            return -10000000000000;
      }

      //Looks at damage dealt / taken
      int allyUnitsLeft = armySizes[playerID] - allyArmySize;
      double allyDamageTaken = armyHealth[playerID] - allyArmyHealth;
      int enemyUnitsLeft = armySizes[!playerID] - enemyArmySize;
      double enemyDamageTaken = armyHealth[!playerID] - enemyArmyHealth;

      double maximum_distance = 10;
      score += (1 / (meanDistance + 1)) * 20; // set to the mean of distance of our units to opponent units, divided by a constant 50 to make it slower than the gameOver score
      ...
      double unitRatio = allyUnitsLeft / enemyUnitsLeft;
      double damageBalance = enemyDamageTaken - allyDamageTaken;

      score += unitRatio * enemyDamageTaken * 4;
      score -= (enemyUnitsLeft / allyUnitsLeft) * allyDamageTaken * 0.25;

...
namespace SGA
{
	PokemonTBSGameRunner::PokemonTBSGameRunner(const GameConfig& newConfig)
		: GameRunner(newConfig)
	{
	}
	
	void PokemonTBSGameRunner::playInternal(std::vector<Agent*>& agents, int /*humanIndex*/)
	{
	...
	void PokemonTBSGameRunner::runInternal(std::vector<Agent*>& agents, GameObserver& observer)
	{
		std::vector<AgentThread> threads(agents.size());
	...
	bool  PokemonTBSGameRunner::checkComputationTime(std::chrono::milliseconds computationTime, int currentPlayerID)
	{
